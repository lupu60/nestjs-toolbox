...
export async function TypeOrmUpsert<T extends ObjectLiteral>(
	...
	const keys: string[] = _keys({
		sampleObject: sampleObject as Record<string, unknown>,
		doNotUpsert,
	});
...
export async function _chunkPromises<T extends ObjectLiteral>({
	...
	for (let i = 0; i < chunkedValues.length; i++) {
		...
		if (returnStatus) {
			...
			const conflictValues = chunkedValues[i]
				.map((item) => (item as Record<string, unknown>)[conflictKey])
				.filter((val) => val !== null);
